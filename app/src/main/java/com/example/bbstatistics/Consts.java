package com.example.bbstatistics;

public final class Consts {
    public static final String TAG = "BBStat";
    public static final int ACTIVITY_REQUEST_NEW_GAME = 1;
    public static final int ACTIVITY_REQUEST_EDIT_GAME = 2;
    public static final int ACTIVITY_REQUEST_START_GAME = 3;
    public static final String ACTIVITY_REQUEST_DATA_GAMEID_KEY = "gameIdKey";
    public static final String ACTIVITY_RESULT_NEW_GAME_KEY = "newGameResultData1";
}
